...
import {
  BookmarkValidation,
  type BookmarkValidationQuery,
} from "../validation/bookmark.validation";
import type { ProductQueryResponse } from "../validation/product.validation";
import Validation from "../validation/validation";

const database = prismaClient.userBookmarkedProduct;
const validation: typeof BookmarkValidation = BookmarkValidation;
  ...
  static async QUERY(
    ...
    const [products, totalFiltered, totalAll] = await Promise.all([
      ...
      prismaClient.product.count({
        where: { bookmarkedByUsers: { some: { userId } }, ...where },
      }),
      prismaClient.product.count({
        where: { bookmarkedByUsers: { some: { userId } } },
      }),
    ]);
